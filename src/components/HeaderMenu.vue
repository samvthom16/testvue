<template>
  <div class="2xl:container 2xl:mx-auto sticky top-0 z-50">
    ...
    <div class="bg-white shadow-lg py-5 px-7">
      <nav class="flex justify-between">
        <div class="flex items-center space-x-3 lg:pr-16 pr-6">
          ...
          <h2 class="font-normal text-2xl leading-6 text-gray-800">InPursuit</h2>
        </div>
        <!-- For medium and plus sized devices -->
        <!--ul class="md:flex flex-auto space-x-2">
          <li v-for='menuItem in menuItems' :key='menuItem.to'>
            <router-link :to="menuItem.to" active-class="bg-orange text-white border-orange" class='focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray text-black border border-white bg-white cursor-pointer px-3 py-2.5 font-normal text-xs leading-3 shadow-md rounded'>
              {{ menuItem.label }}
            </router-link>
          </li>
        </ul-->
